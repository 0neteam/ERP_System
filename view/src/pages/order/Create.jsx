...
const Create = () => {
  ...
  const orderEvent = () => {
    PUT('/stg/order', {items: orders, status: 1}).then(res => {
    ...
    <>
      <section className="container">
        ...
        <div className="d-flex justify-content-end gap-2 mt-4 mb-4">
          <button type="button" className="btn btn-outline-success" onClick={orderEvent} disabled={orders.length == 0}>발주 신청</button>
          <a type="button" className="btn btn-outline-secondary" href="/order">취소</a>
        </div>
      </section>
